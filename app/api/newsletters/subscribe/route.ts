...
export async function POST(request: Request) {
  ...
  const token = process.env.HIPOSTA_PUBLISHER_REJIMDE_TOKEN?.trim() || '';
  if (!token) {
    return NextResponse.json(
  ...
  try {
    const response = await fetch(`${HIPOSTA_CORE_API_URL}/subscriptions/batch`, {
      ...
      headers: {
        ...
        'X-Hiposta-Publisher': REJIMDE_PUBLISHER,
        'X-Hiposta-Client-IP': ip,
        'X-Hiposta-Client-User-Agent': request.headers.get('user-agent') || 'Rejimde Web',
      },
      body: JSON.stringify({
        email,
        newsletters: requested,
        consent: true,
        consent_version: 'rejimde-newsletter-v1',
        source,
        locale: 'tr',
      }),
      cache: 'no-store',
