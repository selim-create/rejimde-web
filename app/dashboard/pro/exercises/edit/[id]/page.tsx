...
import { getPlan, updateExercisePlan, uploadMedia } from "@/lib/api"; // getPlan'ı burada kullanabiliriz çünkü genel yapı aynı
...
export default function EditExercisePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // --- STATE ---
  const [loading, setLoading] = useState(true); // Veri yüklenirken true
  ...
  useEffect(() => {
    const loadPlan = async () => {
        ...
        try {
            // getPlan fonksiyonu ID ile veri çekiyor (rejimde_plan veya rejimde_exercise fark etmeksizin meta verilerini alabiliriz)
            ...
            if (res.success && res.data) {
                ...
                if (data.meta) {
                    ...
                    setCaloriesBurn(data.meta.calories || "");
                    setDuration(data.meta.duration || "30");
                    setScoreReward(data.meta.score_reward || "50");
                    
                    // Tip güvenliği için 'as any' kullanıyoruz veya opsiyonel erişim sağlıyoruz
                    // Çünkü getPlan'dan dönen meta tipinde exercise_category tanımlı olmayabilir
                    const metaData = data.meta as any;
                    setSelectedCategory(metaData.exercise_category || metaData.diet_category || "Güç Antrenmanı");
                    ...
                    // Ekipman Listesi (Meta'dan veya root'tan)
                    // Not: any kullanarak tip hatasını geçiyoruz, safeParse ile string gelirse parse ediyoruz
                    let metaEquipment = metaData.equipment_list;
                    if (typeof metaEquipment === 'string') {
                        try { metaEquipment = JSON.parse(metaEquipment); } catch(e) { metaEquipment = []; }
                    }
                    if (metaEquipment && Array.isArray(metaEquipment)) {
                        setEquipmentList([...metaEquipment, ""]);
                    }

                    // Etiketler
                    let metaTags = metaData.tags;
                    if (typeof metaTags === 'string') {
                        try { metaTags = JSON.parse(metaTags); } catch (e) { metaTags = []; }
                    }
                ...
                // Plan Data (Günler ve Egzersizler)
                if (data.plan_data) {
                    // String gelirse parse et
                    let pData = data.plan_data;
                ...
                showModal("Hata", "Plan bulunamadı.", "error", () => router.push("/dashboard/pro"));
            }
        } catch (error) {
            console.error("Yükleme hatası:", error);
            showModal("Hata", "Veriler yüklenirken bir sorun oluştu.", "error");
        } finally {
            setLoading(false);
        }
    };
  ...
  const handleUpdate = async () => {
      ...
      try {
        const res = await updateExercisePlan(parseInt(id), planData);
        
        if (res.success) {
            showModal("Başarılı", "Egzersiz planı başarıyla güncellendi! ✅", "success", () => {
                if (res.data && res.data.slug) router.push("/exercises/" + res.data.slug);
            });
        } else {
            showModal("Hata", res.message || "Güncelleme başarısız.", "error");
        }
      } catch (err) {
      ...
      return (
          <div className="min-h-screen bg-slate-50 flex items-center justify-center">
              <div className="flex flex-col items-center gap-4">
                  <i className="fa-solid fa-circle-notch animate-spin text-4xl text-blue-600"></i>
